...
There are two checks that payload needs to pass, first check makes sure the payload does not contain the words import, chr, os etc. and the second check is quite basic and we can ignore it for now. Let's create the first payload with intention to remove the bounderies for the second payload, unlike in C when running an exec command we can interact with the variables of the progam, this means we can try to overwrite the banned or the search_func variables. Global python variables are saved in the globals dictionary, when printing it (payload = print(globals)) we recive:
...
Great! The second payload can be whatever we want it to be.
Now for the second payload. We cannot use open since the open function is part of the python __builtins__, a function that is built in in the laguage, and when running exec with '__builtins__': {} all of our built in function s were removed. So what next? After a quick google search "How to restore builtins" you will arrive to this stackoverflow answer that says that each python module contains builtin for itself! In the globals we see we can access the re module lets try to use the open function from this module's builtins:
